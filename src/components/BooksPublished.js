...
import { BookOpen, Star, ArrowRight, Library, Loader, Play } from 'lucide-react';
import { db } from '../firebase/firebaseConfig';
import { doc, getDoc } from 'firebase/firestore';

// Theme configuration (kept the same)
const theme = {
    gradients: {
...
const BooksSection = ({ homepageData }) => {
    ...
    const handleThumbnailClick = (book) => {
        ...
        // If no promo_video_url, do nothing, as requested.
    };
        ...
        return (
            <section className={`py-16 md:py-20 bg-gradient-to-br ${theme.gradients.light}`}>
                <div className="container mx-auto px-4 max-w-7xl">
                    <div className="flex flex-col items-center justify-center py-20">
                        <BookOpen className="w-16 h-16 text-gray-300 mb-4" />
                        <h3 className="text-2xl font-bold text-gray-900 mb-2">No books available for promotion</h3>
                                    ...
                                    {book.promo_video_url && (
                                        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                            <div className="bg-white/90 p-4 rounded-full shadow-2xl backdrop-blur-sm transform group-hover:scale-110 transition-transform">
                                                <Play className={`w-8 h-8 ${theme.text.brand} fill-current`} />
                                            </div>
                                        </div>
                                    )}
                                </div>
