...
import { FileText, Calendar, ArrowRight, Clock, Library, TrendingUp, Loader } from 'lucide-react';
...
const ArticlesSection = () => {
  ...
  // Cache helpers
  const CACHE_KEY = 'articles_section_cache';
  const CACHE_TIMESTAMP_KEY = 'articles_section_cache_timestamp';
  ...
  const loadArticles = async (useCache = true) => {
    try {
      ...
      // Fetch from Firebase - Latest 8 articles only
      const q = query(
      ...
      const fetchedArticles = snapshot.docs.map(doc => {
        ...
        return {
          ...
          image: data.preview_image_url || 'https://via.placeholder.com/600x400?text=No+Image',
          category: data.article_category || 'General',
